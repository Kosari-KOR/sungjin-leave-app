...
def render_user_dashboard(user_row, selected_year):
    ...
    if df_leave is not None:
        ...
        # 💡 핵심: 엑셀의 '2024총연차' 같은 열에서 값을 직접 가져옴
        excel_col_name = f"{selected_year}총연차"
        
        # 엑셀에 해당 연도 열이 있고 값이 들어있으면 그 값을 사용
        if excel_col_name in user_row and pd.notna(user_row[excel_col_name]):
            total_days = float(user_row[excel_col_name])
        else:
            total_days = 0.0 # 정보가 없으면 0으로 표기
